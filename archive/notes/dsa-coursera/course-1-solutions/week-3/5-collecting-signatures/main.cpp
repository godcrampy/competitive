...
std::vector<long> intersection_of_arrays(std::vector<long> a,
                                         std::vector<long> b) {
  std::vector<long> final;
  for (auto number : a)
    if (std::binary_search(b.begin(), b.end(), number)) final.push_back(number);
  return final;
}

std::vector<long> pair_to_vector(std::pair<long, long> pair) {
  std::vector<long> final;
  for (auto i = pair.first; i <= pair.second; ++i) final.push_back(i);
  return final;
}
...
std::vector<long> optimal_points(std::vector<std::pair<long, long>> &set) {
  ...
  std::vector<long> temp =
      pair_to_vector(set[0]);  // we'll add points to this array that are
                               // allowed in certain case
  for (auto pair : set) {
    // some points already exist, see if the current pair will fit in it
    auto intersection = intersection_of_arrays(
        temp, pair_to_vector(pair));  // intersection of current pair points and
                                      // points available in temp
...
int main(int argc, char const *argv[]) {
  ...
  for (auto point : output) std::cout << point << " ";
  std::cout << std::endl;
  return 0;
}
