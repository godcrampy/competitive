...
typedef struct item {
  ...
  bool operator>(const item &item2) const {
    return (static_cast<double>(value) / weight) >
           (static_cast<double>(item2.value) / item2.weight);
  }
} item;

double maximum_value(std::vector<int> weights, std::vector<int> values,
                     int weight_limit) {
  ...
  while (weight_limit > 0) {
    if (iterator->weight <= weight_limit) {
      ...
      value += static_cast<double>(iterator->value) * weight_limit /
               iterator->weight;
      weight_limit = 0;
    }
    ++iterator;
...
int main(int argc, char const *argv[]) {
  ...
  std::cout << std::fixed << std::setprecision(4)
            << maximum_value(weights, values, limit) << std::endl;
  return 0;
}
