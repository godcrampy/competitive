...
inline auto last_set_bit(int number) { return number - (number & (-number)); }

inline auto parent(int number) { return number + (number & (-number)); }
...
auto make_BIT(std::vector<int> &array, std::vector<int> &BIT) {
  ...
  for (auto i = 1; i < size; ++i) {
    ...
    for (auto j = start + 1; j <= i; ++j) sum += array.at(j);
    BIT.push_back(sum);
  }
}
