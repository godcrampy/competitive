...
int get_hotness(int men_rating[], int women_rating[], int length) {
  ...
  for (int i = 0; i < length; i++) sum += men_rating[i] * women_rating[i];
  return sum;
}
...
int main(int argc, char const *argv[]) {
  ...
  for (int _ = 0; _ < t; _++) {
    ...
    for (int i = 0; i < number_of_contestants; i++) cin >> men_ratings[i];
    for (int i = 0; i < number_of_contestants; i++) cin >> women_rating[i];
    cout << get_hotness(men_ratings, women_rating, number_of_contestants)
         << endl;
  }
  return 0;
}
