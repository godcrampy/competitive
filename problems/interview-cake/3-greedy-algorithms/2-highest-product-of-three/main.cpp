...
int productOfThree(const vector<int>& v) { return v[0] * v[1] * v[2]; }
...
int highestProductOf3(const vector<int>& vectorOfInts) {
  ...
  for (int i = 2; i < length; ++i) {
    ...
    highestProductOf2 =
        max(max(highestProductOf2, current * highest), current * lowest);

    lowestProductOf2 =
        min(min(lowestProductOf2, current * highest), current * lowest);
